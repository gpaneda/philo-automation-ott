...
export async function POST(request: Request) {
  console.log('POST /api/execute received');
  try {
    const body = await request.json();
    console.log('Request body:', body);

    if (!body.suiteId || !body.testId) {
      return NextResponse.json({ 
        success: false,
        error: 'Missing suiteId or testId in request'
      }, { status: 400 });
    }
    ...
    try {
      console.log(`Executing test: ${body.testId} in suite: ${testPath}`);
      // Change to the root directory where tests are located
      const rootDir = path.resolve(process.cwd(), '..');
      const command = `cd ${rootDir} && NODE_OPTIONS=--experimental-vm-modules jest ${testPath} -t "${body.testId}"`;
