...
exports.ElementIdHelper = void 0;
const adb_helper_1 = require("./adb.helper");
...
class ElementIdHelper {
     ...
     */
    static async createSimplifiedDump() {
        try {
            const content = await this.readUiDump();
            const result = await (0, xml2js_1.parseStringPromise)(content);
            let simplified = '';
            const processNode = (node, depth = 0) => {
                ...
                // Process child nodes
                if (node.node) {
                    node.node.forEach((child) => processNode(child, depth + 1));
                }
            };
     ...
     */
    static async getAllElementIds() {
        try {
            // Dump the current UI hierarchy
            const uiDump = await adb_helper_1.AdbHelper.dumpUiHierarchy(this.UI_DUMP_PATH);
     ...
     */
    static async getElementSelector(elementName) {
        try {
...
exports.ElementIdHelper = ElementIdHelper;
ElementIdHelper.UI_DUMP_PATH = path.join('src', 'config', 'ui_dump.xml');
ElementIdHelper.FORMATTED_UI_DUMP_PATH = path.join('src', 'config', 'ui_dump_formatted.xml');
ElementIdHelper.SIMPLIFIED_DUMP_PATH = path.join('src', 'config', 'txt_files', 'ui_dump_simplified.txt');
